...
def process_and_plot(file_path, filter_column, metrics):
    ...
    for metric in metrics:
        ...
        plt.savefig(f'history/overall_average_{metric}.png')
        plt.show()
        ...
        plt.savefig(f'history/per_filter_average_{metric}.png')
        plt.show()

# Paths to your preprocessed CSVs (update these paths to your actual files)
file_paths = {
    'SRP_analysis': 'results/SRP_analysis_all_filters.csv',
    'SRP+Bias_analysis': 'results/SRP+Bias_analysis_all_filters.csv',
    'SRP+Ground_analysis': 'results/SRP+Ground_analysis_all_filters.csv'
}
